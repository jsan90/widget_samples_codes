import {
  ...
  getAppStore, MutableStoreManager, ImmutableObject, dataSourceUtils, FeatureLayerDataSource, UseDataSource, JSAPILayerMixin
} from 'jimu-core'
...
export default class MapBase extends React.PureComponent<Props, State> {
        ...
        if (this.props.baseWidgetProps.mutableStateProps) {
          ...
          if (tempJimuMapView) {
            tempJimuMapView.whenJimuMapViewLoaded().then(() => {
              setTimeout(() => {
                this.handleAction(this.props.baseWidgetProps.mutableStateProps, this.mapView, tempJimuMapView)
      ...
      this.analysisSceneView().then(() => {
        ...
        if (this.props.baseWidgetProps.mutableStateProps) {
          ...
          if (tempJimuMapView) {
            tempJimuMapView.whenJimuMapViewLoaded().then(() => {
              setTimeout(() => {
                this.handleAction(this.props.baseWidgetProps.mutableStateProps, this.sceneView, tempJimuMapView)
                this.isFirstReceiveMessage = false
              }, this.isFirstReceiveMessage ? 500 : 0
              )
            })
  ...
  goHome = async (useAmination?: boolean): Promise<any> => {
    ...
    if (this.getDsJsonFromDsId(this.props.dataSourceId).type === DataSourceTypes.WebMap) {
      if (this.mapView && widgets[this.props.baseWidgetProps.id]) {
    ...
    if (this.getDsJsonFromDsId(this.props.dataSourceId).type === DataSourceTypes.WebScene) {
      if (this.sceneView && widgets[this.props.baseWidgetProps.id]) {
  ...
  queryExtentFromQueryParams = (mapBaseView: __esri.MapView | __esri.SceneView, dataSourceIds: string[], useDataSources: UseDataSource[]) => {
    //const dataSource = this.dsManager.getDataSource(dataSourceId) as FeatureLayerDataSource
    //const queryParams: SqlQueryParams = dataSource.getCurrentQueryParams()
   ...
   */
  handleAction = (mutableStateProps: ActionRelatedProps, mapBaseView: __esri.MapView | __esri.SceneView, jimuMapView: JimuMapView) => {
    if (mutableStateProps.zoomToFeatureActionValue) {
      if (mutableStateProps.zoomToFeatureActionValue.relatedWidgets &&
        ...
        if (zoomToFeatureValue.type === 'zoom-to-extent') {
          ...
          this.queryExtentFromQueryParams(mapBaseView, zoomToFeatureValue.dataSourceIds, zoomToFeatureValue.useDataSources).then((result) => {
            let target
            if (result.count === 0) {
              return
        ...
        } else if (zoomToFeatureValue.type === 'zoom-to-array-graphics') {
          tempMapBaseView.isReceiveExtentChange = true
          zoomToUtils.zoomTo(tempMapBaseView, zoomToFeatureValue.arrayFeatures, zoomToFeatureValue.zoomToOption).then(() => {
            relatedWidgets.push(this.props.baseWidgetProps.id)
            const extentMessage = new ExtentChangeMessage(this.props.baseWidgetProps.id, tempMapBaseView.extent)
            extentMessage.setRelatedWidgetIds(relatedWidgets)
            this.props.onExtentChanged(this.props.dataSourceId, extentMessage)
        ...
        } else if (zoomToFeatureValue.type === 'zoom-to-layers') {
          const dataSourceIds = zoomToFeatureValue.dataSourceIds
          getLayersFromDataSourceIds(dataSourceIds).then(layers => {
            tempMapBaseView.isReceiveExtentChange = true
            zoomToUtils.zoomTo(tempMapBaseView, layers, zoomToFeatureValue.zoomToOption).then(() => {
              relatedWidgets.push(this.props.baseWidgetProps.id)
              const extentMessage = new ExtentChangeMessage(this.props.baseWidgetProps.id, tempMapBaseView.extent)
              extentMessage.setRelatedWidgetIds(relatedWidgets)
              this.props.onExtentChanged(this.props.dataSourceId, extentMessage)
          ...
          const dataSource = this.dsManager.getDataSource(zoomToFeatureValue.dataSourceId) as unknown as JSAPILayerMixin & FeatureLayerDataSource
          let layerPromise
          if (dataSource?.layer) {
          ...
          layerPromise.then(layer => {
            ...
            processZoomToFeatures(tempMapBaseView, target.layer, (target && target.graphics) ? target.graphics : target).then(
              graphics => {
                ...
                tempMapBaseView.isReceiveExtentChange = true
                if (!zoomToFeatureValue.zoomToOption) {
                  zoomToFeatureValue.zoomToOption = {}
                }
                zoomToFeatureValue.zoomToOption.queryParams = (dataSource?.getCurrentQueryParams && dataSource.getCurrentQueryParams()) || ''
    ...
    if (mutableStateProps.panToActionValue) {
      if (mutableStateProps.panToActionValue.relatedWidgets &&
      ...
      } else if ((mutableStateProps.panToActionValue?.value as any)?.type === 'pan-to-layer') {
        const panToValue = mutableStateProps.panToActionValue.value as any
      ...
      } else if ((mutableStateProps.panToActionValue?.value as any)?.type === 'pan-to-layers') {
        const panToValue = mutableStateProps.panToActionValue.value as any
        ...
        const relatedWidgets = mutableStateProps.panToActionValue.relatedWidgets
          ? mutableStateProps.panToActionValue.relatedWidgets
          : []
        this.panToGeometry(panToValue.geometries, mapBaseView, relatedWidgets)
      }
